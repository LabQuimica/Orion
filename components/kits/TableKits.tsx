...
import { fetchKits } from "./fetching/selectKit";
import ModalUpdateKit from "./modal/modalEditKit";

import ModalDeleateKit from "./modal/modalDeleateKIt";
...
const columnHelper = createColumnHelper<Kits>();
...
const TableKits = () => {
  ...
  const handleUpdateData = async (updatedData: any) => {
    ...
    const updatedDataIndex = data.findIndex(
      (item: any) => item.id_kits === updatedData.id_kits
    );
    const newData = [...data];
  ...
  const columns = [
    ...
    columnHelper.accessor("cantidad_kits", {
      header: "Cantidad Kits",
      cell: (info) => info.renderValue(),
    }),
        ...
        return (
          <Stack className="justify-center gap-4" direction={"row"}>
            ...
            >
              ...
              {isOpenDelete && (
                <ModalDeleateKit
                  resultado={row.original.id_kits}
